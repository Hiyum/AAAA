...
class Config:
    ...
    # ⚠ 소액 데모 전용 우회 스위치: True면 하드캡을 넘어도 '최소랏 1건'으로
    # 강행한다 (래더 축소). GOLD 0.01랏 최소 거래의 리스크가 $100 계좌에서
    # 8~13%라는 사실은 변하지 않는다 — 연속 손실 몇 번이면 계좌가 죽는
    # 수학을 알고 켜는 것. 실계좌에서는 절대 켜지 말 것 (권장: 잔고 $1,000+).
